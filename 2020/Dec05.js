...
const readFile = fs.readFileSync('./Dec05Input.txt', 'utf-8');

const data = readFile.split('\n');

const trimmedData = data.map(el => el.trim());
console.log(trimmedData)

//find the largest seat ID, Row * 8 * Column
...
trimmedData.forEach(seat => {
  ...
  let range = 0;
  for(let i = 0; i < 7; i++){
    range = Math.ceil((highestRow - lowestRow) / 2);
    if(seat[i] === 'F'){
      highestRow = highestRow - range;
      rowValue = highestRow;
    }
    if(seat[i] === 'B'){
      lowestRow = lowestRow + range;
      rowValue = lowestRow;
    }
  }
  let calculatedId = rowValue * 8 + columnDecoder[seat.substr(7, 3)]
  console.log(columnDecoder[seat.substr(7, 3)])
