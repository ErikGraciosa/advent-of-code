...
//Pseudocode
//Find the accumulator value before infinite loop happens.
//find when the infinite loop happens report out the acc value
  //acc needs to add the value and increase the next location by 1
  //jmp needs to increase/decrease the next location by jmp value
  //nop needs to increase the next location by 1

const readFile = fs.readFileSync('./Dec08aInput.txt', 'utf-8');

// const groupAnswers = readFile.split('\r\n\r\n');

const data = readFile.split('\n');

//remove the \r in strings
const trimmedData = data.map(instruction => instruction.substr(0, instruction.length - 1));
