...
const minArray = [];
const maxArray = [];
const keyLetterArray = [];
const passwordArray = [];
passwords.forEach(password => {
    newArray.push(password.split('-', 1));
    const rangeArray = password.split(' ');
    const rangeArrayElements = rangeArray[0].split('-');
    maxArray.push(rangeArrayElements[1]);
    keyLetterArray.push(rangeArray[1][0]);
    passwordArray.push(rangeArray[2]);
});

const splitPasswords = [];
passwordArray.forEach(password => {
    splitPasswords.push(password.split(''));
})

const numberOfKeysArray = [];
splitPasswords.forEach((password, index) => {
    let numberOfKeys = 0;
    password.forEach(letter => {
        if(letter === keyLetterArray[index]) {
            numberOfKeys++;
        }
    })
    numberOfKeysArray.push(numberOfKeys);
})

let validPasswords = 0;
numberOfKeysArray.forEach((numOfKeys, index) => {
    if(numOfKeys >= newArray[index][0] && numOfKeys <= maxArray[index]) {
        validPasswords++;
    }
})

let newReq2 = 0;
splitPasswords.forEach((splitPassword, index) => {
    let isLower = splitPassword[(newArray[index][0]) - 1] === keyLetterArray[index];
    let isUpper = splitPassword[(maxArray[index]) - 1] === keyLetterArray[index];
    ...
    // if(splitPassword[newArray[index][0] - 1] === keyLetterArray[index] && splitPassword[maxArray[index][0] - 1] !== keyLetterArray[index]) {
    //     newReq2++;
    // }
    // if(splitPassword[newArray[index][0] - 1] !== keyLetterArray[index] && splitPassword[maxArray[index][0] - 1] === keyLetterArray[index]) {
    //     newReq2++;
