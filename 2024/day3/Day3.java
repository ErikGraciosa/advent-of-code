...
public class Day3 {
    ...
    public static void main(String[] args) {
        ...
        StringBuilder inputPart2 = new StringBuilder(input);

        //Matching pattern, thanks chatGPT
        ...
        int total = sumOfMatches(matches);

        System.out.println("Part1 " + total); //Answer 1166630675

        //Part 2
        //Strategy is to only get the mult(*,*) between a do() and dont() by string manipulation but deleting the original string between instances of don't() and do()
        ...
        while (index != -1) {
            index = input.indexOf(targetDo, index + 1); // Search from the next character
            if (index != -1) {
                matchList.add(index);
            }
        }
        ...
        int j = 0;
        while (j != -1) {
            j = input.indexOf(targetDont, j + 1); // Search from the next character
            if (j != -1) {
                matchList.add(j);
            }
        }
        
        //Sort the matchlist of instances of don't() and do()
        matchList.sort(null);

        // check each index for a dont followed by a do, when those are found splice out the contents.
        List<int[]> rangesToDelete = new ArrayList<>();

        // Collect ranges to delete
        for (int i = 0; i < matchList.size(); i++) {
            int start = matchList.get(i);
            if (inputPart2.substring(start, start + 7).equals("don't()")) {
                for (int k = i + 1; k < matchList.size(); k++) {
                    int end = matchList.get(k);
                    if (inputPart2.substring(end, end + 4).equals("do()")) {
                        rangesToDelete.add(new int[]{start, end + 4}); // Record range
                        i = k; // Skip to the end of this match
                        break;
                    }
                }
            }
        ...
        // Delete ranges in reverse order, avoiding index issues of deleting indexes.
        for (int i = rangesToDelete.size() - 1; i >= 0; i--) {
            int[] range = rangesToDelete.get(i);
            inputPart2.delete(range[0], range[1]);
        }

        // Create a matcher to find matches in the input string
        Matcher matcher2 = regex.matcher(inputPart2);
        ...
        // Find and add matches to the list
        while (matcher2.find()) {
            matches2.add(matcher2.group()); // Add the full match to the list
        }

        int total2 = sumOfMatches(matches2);

        System.out.println("Part2 " + total2); //Answer 93465710

    }

    public static int sumOfMatches(List<String>matches) {
        //Loop through results
        //Strip the list of strings to 1st and 2nd number
        ...
        for (int i = 0; i < matches.size(); i++) {
            String result = matches.get(i).substring(4, matches.get(i).length() - 1);
            String[] splitResults = result.split(",");
            total = total + Integer.parseInt(splitResults[0]) * Integer.parseInt(splitResults[1]);
        }
